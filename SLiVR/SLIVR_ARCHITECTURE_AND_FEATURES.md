...
The initial map is a Lafayette city-center scouting region rather than a single campus map. It covers seven operational areas from the original inventory and the owner-approved LaSEL addition of 2026-09-20 (D064):
...
| UL Research Park | 0 | 1 | LaSEL / Antoun Hall is a future candidate with no capture | Separate research facility at 439 Eraste Landry Road; access authority and scan zones need validation |

This produces **18 catalog records: 11 with current Treedis entry links and seven future candidates** in catalog 1.1.0. The 11 current records use six distinct Treedis experience IDs. Exact model coverage, capture dates, access rights and current physical condition have not been verified. The links are project inventory, not proof that every relevant room or exterior is captured.
...
#### F06-SA — Scouting Assessments · Phase 3 (D068)

Owner-approved essential extension of Project Workspace: dated project/location assessments using the eight-section checklist, after candidate creation and before comparison. One editor is accessible from Projects, the active-project dossier and Immersive. Include explicit unknown/evidence states, repeat assessments and revision history, owned photo/video attachments, existing panorama viewing, supported bookmark links, requirement evidence and validated transfer. Completion is not suitability or filming permission. Full records, migration, source-reuse and exit contracts are in [Scouting Assessments plan](SCOUTING_ASSESSMENTS_PLAN.md). Tests 206–215 are planned, not passed.

Guided capture/stitching/refinement, GPS, compass, microphone, dedicated offline phone tools and operational permissions remain Phase 7. Compliance in the essential checklist is limited to validation prompts; private contacts remain excluded. No new mode or product family is added.
...
| **3 — Project Workspace** | Add local productions, multiple scene requirements, candidates, dated scouting assessments with evidence/media and revision history, suitability ratings, comparison, preferred and backup choices, decision rationale, links to locations/bookmarks/shot scenes, autosave and validated JSON import/export. | The Phase 3 portions of tests 206-212 and 214-215 pass. A user can compare at least two candidates for a scene, preserve unknowns as unknown, choose a preferred location and backup, and reopen the decision with its evidence intact; reload and JSON transfer preserve stable IDs and relationships; failed persistence retains the working state and offers emergency JSON recovery. |
...
- **Phase 7 — Field production:** scout-visit planning, phone field tools (guided 360 capture, stitching/refinement, GPS, compass, microphone and dedicated offline field workflow), production-logistics maps, permissions, availability, costs and technical-scout handoff.
...
- **Phase 9 — Team and operations:** collaboration and review, capture-library administration, publishing/refresh workflows, wrap/stewardship records, operational hardening and further regional expansion beyond the current catalog.
...
| Expansion | LaSEL is an approved future candidate (D064); broader Acadiana expansion requires reviewed records and approval paths |
