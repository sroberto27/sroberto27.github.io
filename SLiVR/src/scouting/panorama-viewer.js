...
*/
  ...
  function open(url, opts) {
    ...
    let disposed = false, failed = false; let cleanup = () => {};
    const close = () => { if (disposed) return; disposed = true; cleanup(); document.removeEventListener('keydown', escHandler); overlay.remove(); if (opts.onClose) opts.onClose(); };
    overlay.querySelector('.pano-viewer-close').addEventListener('click', close);
    const escHandler = e => { if (e.key === 'Escape') close(); if (e.key === 'Tab') { e.preventDefault(); const target = document.activeElement === overlay.querySelector('button') ? overlay.querySelector('canvas') : overlay.querySelector('button'); (target ?? overlay.querySelector('button')).focus(); } };
    ...
    img.onload = () => { if (!disposed) { try { startGl(gl, canvas, img); } catch { canvas.remove(); openFallback(stage, url); } } };
    canvas.addEventListener('webglcontextlost', e => { e.preventDefault(); if (failed || disposed) return; failed = true; cleanup(); canvas.remove(); openFallback(stage, url); });
    img.src = url;

    function startGl(gl, canvas, image) {
      const prog = gl.createProgram();
      const vertexShader = compile(gl, gl.VERTEX_SHADER, VERT), fragmentShader = compile(gl, gl.FRAGMENT_SHADER, FRAG);
      gl.attachShader(prog, vertexShader); gl.attachShader(prog, fragmentShader);
      gl.linkProgram(prog);
      ...
      canvas.tabIndex = 0;
      canvas.setAttribute('aria-label', 'Panorama: arrow keys look around, plus/minus zoom');
      canvas.addEventListener('keydown', e => { if (['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','-'].includes(e.key)) { e.preventDefault(); yaw += e.key === 'ArrowRight' ? .1 : e.key === 'ArrowLeft' ? -.1 : 0; pitch = Math.max(-1.5, Math.min(1.5, pitch + (e.key === 'ArrowUp' ? .1 : e.key === 'ArrowDown' ? -.1 : 0))); fovDeg = Math.max(30, Math.min(100, fovDeg + (e.key === '+' ? -5 : e.key === '-' ? 5 : 0))); render(); } });
      resize();
      render();
