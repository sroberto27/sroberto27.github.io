/* ===========================================================
   SCSU METAVERSITY — App (no Treedis / no street view)
   -----------------------------------------------------------
...
/* Builds the list of things to preload and tracks progress.
   ...
   This version (no Treedis) only preloads images — the satellite
   SVG plus every entry in config.imageMap. */
function preloadAllAssets(onProgress) {
  ...
  // If there's nothing to preload, resolve immediately so the
  // splash doesn't sit there with no progress to report.
  if (imageUrls.length === 0) {
    try { onProgress && onProgress(0, 0); } catch (_) {}
...
function updateSplashProgress(done, total) {
  ...
  if (total === 0) {
    ...
    node.textContent = "Loading " + done + "/" + total + "…";
  }
}
...
async function boot() {
  ...
  // splash. preloadAllAssets() covers the satellite SVG and every
  // entry in config.imageMap. It reports progress to the splash
  // counter as each asset completes. Every underlying promise
  // has its own 10s timeout so it cannot hang; a final 15s hard
  // cap reveals the app no matter what as an extra safety net.
  ...
  requestAnimationFrame(() => {
    ...
    // Matches the .4s CSS opacity transition on .splash.
    setTimeout(() => { el.splash.style.display = "none"; }, 500);
    scheduleMapRefresh({ delay: 80 });
  });
}

// Kick off boot immediately. The splash stays visible until the
// satellite image and building photos have finished loading
// (see preloadAllAssets inside boot).
boot().catch((err) => {
