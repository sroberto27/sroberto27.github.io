/* === LSU Death Valley Experience — Part 6: Details panel + selection (sec 7, 8) === */
/* -----------------------------------------------------------
   7. Details panel
   ----------------------------------------------------------- */
...
/* -----------------------------------------------------------
   8. Selection + focus
   -----------------------------------------------------------
   Ported from the Leaflet build. `selectFeature()`'s signature
   changed shape (was `(layer, kind, opts)`, now `(sel, kind, opts)`
   where `sel = {sourceId, featureId, feature}`) — every caller
   (js/07-layer-builders.js click/marker handlers, js/08-tourbar.js,
   js/09-sidebar-search.js) was updated to match. This is still the
   ONE seam every selection path goes through, same as before.
   ----------------------------------------------------------- */
function resetFeatureState(sel) {
  if (!sel) return;
  map.setFeatureState({ source: sel.sourceId, id: sel.featureId }, { selected: false });
}
...
/* Compute the padding to use when flying to a selected feature.
   ...
   MapLibre's fitBounds() padding option takes a number or a
   {top,bottom,left,right} object — not Leaflet's paddingTopLeft/
   paddingBottomRight array-pair shape. */
function focusPaddingFor() {
  if (!isMobile()) return { padding: 80 };
  ...
  return { padding: { top: 24, left: 24, right: 24, bottom: bottomPad } };
}

function selectFeature(sel, kind, { focus = false } = {}) {
  if (selectedFeature &&
      (selectedFeature.sourceId !== sel.sourceId || selectedFeature.featureId !== sel.featureId)) {
    resetFeatureState(selectedFeature);
  }
  selectedFeature = { sourceId: sel.sourceId, featureId: sel.featureId, kind, feature: sel.feature };

  const isOffCampus = isOffCampusFeature(sel.feature);

  map.setFeatureState({ source: sel.sourceId, id: sel.featureId }, { selected: true });
  // No Leaflet-style bringToFront() equivalent needed — selection is a
  // color/opacity change via feature-state, not a z-order change.

  renderDetails(sel.feature, kind);
  openDetails();

  /* Off-campus tour stops carry a placeholder polygon at the campus
     edge as a directional indicator only. Flying to it would zoom
     past every real building on the way and confuse the user, so we
     skip the per-feature fly. Instead, we reset to the full campus
     view so the user sees the whole campus plus the directional
     arrow pointing toward the real site — visual confirmation that
     the location isn't on the map. The details panel still opens
     and the user can click Explore to jump straight into the
     Treedis sweep.
     ...
     user can't see. */
  if (focus && sel.feature && sel.feature.geometry && !isOffCampus) {
    const bounds = boundsOfFeature(sel.feature);
    // Let the layout settle before flying so we measure the latest
    // shell width after the details panel state changes.
    const fly = () => {
      refreshMapConstraints({ recenterIfNeeded: false });
      if (!bounds) return;

      if (is3DMode) {
        // Per-stop 3D camera preset (optional `cam3d: {bearing, pitch,
        // zoom}` on the stop's GeoJSON properties — placeholder-now,
        // hand-tune-later, same pattern as stop coordinates). Falls
        // back to a sensible generic tilt when absent, read from
        // config.tour so it's tunable in one place.
        const cam3d = sel.feature.properties && sel.feature.properties.cam3d;
        const center = centerOfBounds(bounds);
        map.easeTo({
          center,
          zoom: (cam3d && cam3d.zoom) ?? config.tour.default3DZoom ?? config.tour.focusZoom,
          pitch: (cam3d && cam3d.pitch) ?? config.tour.default3DPitch ?? 55,
          bearing: (cam3d && cam3d.bearing) ?? config.tour.default3DBearing ?? 0,
          duration: 800
        });
      } else {
        map.fitBounds(bounds, {
          ...
          duration: 550 // ms — MapLibre durations are ms, not Leaflet's seconds
        });
      }
    };
    if (isMobile()) {
    ...
    // we measure the post-panel-open shell width correctly.
    const reset = () => {
      refreshMapConstraints({ recenterIfNeeded: false });
  ...
  const idx = tourStops.findIndex((s) => s.featureId === sel.featureId);
  tourIndex = idx;
  updateTourbar();
  highlightActivePin();
  ...
  // navigateStreetViewToLayer() (js/04-street-view.js, unchanged)
  // still expects a `{feature}`-shaped object, so wrap it.
  if (streetViewActive) {
    navigateStreetViewToLayer({ feature: sel.feature });
  }
}
