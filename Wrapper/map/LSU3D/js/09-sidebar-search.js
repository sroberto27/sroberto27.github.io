/* === LSU Death Valley Experience — Part 9: Sidebar, mobile drawer, search === */
/* Includes sections 13, 13b, 14, 14a, 14b, 15. */
/* -----------------------------------------------------------
   13. Locations sidebar (Figma-style list)
   ----------------------------------------------------------- */
function syncLocationsList() {
  const selectedName = selectedFeature && selectedFeature.feature
    ? cleanName(selectedFeature.feature.properties.name).toLowerCase()
    : null;

  const rows = el.locationsList.querySelectorAll(".location-row");
  rows.forEach((r) => {
    const name = r.dataset.name || "";
    r.classList.toggle("is-active", !!selectedName && selectedName === name);
  });
...
function renderLocationsList() {
  ...
  el.locationsList.querySelectorAll(".location-row").forEach((row) => {
    row.addEventListener("click", () => {
      ...
      const sel = { sourceId: SOURCE_IDS.tours, featureId: stop.featureId, feature: stop.feature };
      ...
      if (streetViewActive) {
        ...
        if (sweepId) {
          ...
          closeStreetView();
          selectFeature(sel, "tour", { focus: true });
        }
        selectFeature(sel, "tour", { focus: false });
        return;
      }
...
/* -----------------------------------------------------------
   ...
   -----------------------------------------------------------
   Populates #allLocationsList from buildingsGeo (the prepped
   building FeatureCollection — see js/07-layer-builders.js:
   prepGeoJSON()). Clicking a row selects that feature on the map
   and opens the details panel, exactly like the Featured rows do
   for tour stops.
   ----------------------------------------------------------- */
function renderAllLocationsList() {
  if (!el.allLocationsList || !buildingsGeo) return;

  // Collect (name, featureId, feature) triples from the building
  // features. We dedupe by lower-cased clean name so duplicate
  // features don't each get their own row.
  const seen = new Map();
  (buildingsGeo.features || []).forEach((f) => {
    ...
    seen.set(key, { name, featureId: f.id, feature: f });
  });
  ...
  el.allLocationsList.querySelectorAll(".location-row").forEach((row) => {
    row.addEventListener("click", () => {
      ...
      // because these come from the buildings source, not tours.
      selectFeature({ sourceId: SOURCE_IDS.buildings, featureId: item.featureId, feature: item.feature },
                    "building", { focus: true });
    });
  });
}
   ...
   function renderSearch(q) {
     ...
     el.searchResults.querySelectorAll(".search-result[data-i]").forEach((node) => {
       node.addEventListener("click", () => {
         ...
         if (!m) return;
         selectFeature({ sourceId: m.sourceId, featureId: m.featureId, feature: m.feature },
                       m.kind, { focus: true });
