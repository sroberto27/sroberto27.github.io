/* === LSU Death Valley Experience — Part 12: Start screen + coachmark walkthrough === */
/* ============================================================
   ...
   stop so the right details panel is populated with real
   content the user can see being pointed to. When the
   walkthrough finishes (final step's "next" or
...
(function setupOnboarding() {
  ...
  /* -- Preferences ------------------------------------------
     Two independent settings, both stored in localStorage:
       • lsu:showStartScreen   — welcome window on boot
       • lsu:showNavInstructions — 3D nav modal on first
     ...
     storage key. --------------------------------------------- */
  const PREF_KEY = "lsu:showStartScreen";
  const NAV_PREF_KEY = "lsu:showNavInstructions";
