...
/* Redesign: tour polygons split by category — parks & recreation
   render in gold, civic/city in blue (config.styles.toursPark vs
   .tours). Category comes from locations.json via getCategory(). */
function isParkFeature(feature) {
  ...
  return /park|recreation|trail|beach/.test(cat);
}
