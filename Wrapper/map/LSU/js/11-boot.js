...
/* Dynamic Esri ImageServer base layer (config.mapMode === "esri-image").
   Unlike addBaseTileLayer() above, this is NOT a cached {z}/{x}/{y} tile
   pyramid — the DOTD 2025 6" RGBI service is a dynamic ImageServer
   ("Single Fused Map Cache": false), so it has to be queried via
   exportImage on every pan/zoom. esri-leaflet's imageMapLayer wraps that.
   bandIds [0,1,2] drop the 4th (near-infrared) band so the image renders
   true-color instead of IR-tinted. */
function addEsriImageryLayer() {
  const img = config.esriImagery || {};
  if (!img.url) {
    console.warn("[metaversity] mapMode is 'esri-image' but config.esriImagery.url is missing.");
    return null;
  }
  if (!(window.L && L.esri && L.esri.imageMapLayer)) {
    console.error("[metaversity] esri-leaflet is not loaded — cannot build the imagery layer.");
    return null;
  }
  ...
  return L.esri.imageMapLayer({
    url: img.url,
    pane: "imagePane",
    bandIds: img.bandIds || [0, 1, 2],
    format: img.format || "jpgpng",
    maxZoom: img.maxZoom ?? 21,
    attribution: img.attribution || "Imagery: Louisiana DOTD"
  }).addTo(map);
}

/* Toggleable streets/buildings/POI-label overlay, built from OpenStreetMap
   data (the only open source dense enough to carry building- and
   business-level labels — parking lot names, campus building names —
   rather than just roads/admin boundaries). Off by default; #layersBtn
   (wired in js/14-redesign.js) toggles it on/off. Plain raster tiles, no
   esri-leaflet dependency. */
function addReferenceOverlay() {
  const ref = config.referenceOverlay || {};
  if (!ref.url) return null;

  return L.tileLayer(ref.url, {
    pane: "buildingsPane",
    opacity: ref.opacity ?? 0.6,
    maxZoom: 19,
    attribution: ref.attribution || ""
  });
  // Intentionally not .addTo(map) — starts hidden; see toggleReferenceOverlay().
}
async function boot() {
  ...
  if (!dataBounds.isValid()) {
    ...
    dataBounds = L.latLngBounds([30.405, -91.192], [30.419, -91.176]);
  }

// Base map: esri-image (dynamic DOTD ImageServer) is the production default.
// "tiles" (local pre-rendered XYZ) is kept as a fallback mode for dev use.
// Either way, the raster map is already georeferenced — we do NOT compute
// image bounds from the polygons.
if (config.mapMode === "esri-image" || config.mapMode === "tiles") {
  baseTileLayer = (config.mapMode === "esri-image")
    ? addEsriImageryLayer()
    : addBaseTileLayer();

  referenceOverlayLayer = addReferenceOverlay();
