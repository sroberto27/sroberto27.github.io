...
/* config.mapMode "esri-image" (dynamic DOTD ImageServer) is treated the
   same as "tiles" everywhere below — both are already georeferenced base
   layers that fit/constrain the same way; only the layer construction in
   11-boot.js differs between them. */
function isGeoreferencedMapMode() {
  return config.mapMode === "tiles" || config.mapMode === "esri-image";
}
/* -----------------------------------------------------------
...
function resetCampusView(animate = false) {
  ...
  // Tile / esri-image mode: fit to the campus/vector bounds, not to a
  // fake image box.
  if (isGeoreferencedMapMode()) {
    const opts = {
      padding: [24, 24],
      animate
    };
