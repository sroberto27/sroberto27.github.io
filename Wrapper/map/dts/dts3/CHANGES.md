...
and the DTS portfolio (`DTS_Portfolio_Follow-up.pdf`). Text/dialog draws on the
design-rationale PDF (text only — no images taken from it, per instruction).

## New per-example fields in `config.js` → `examples`
- `media` — the window's main experience pane. `type: "treedis"` loads that
  project's live tour; `type: "vimeo"` embeds the best directly-related project
  video when no Treedis experience exists (per the curation instruction).
- `links` — "More from this project": related tours/videos from the inventory.
- `gallery` — real project imagery, extracted from the portfolio PDF into
  `assets/portfolio/` (SCSU campus, Downtown Lafayette, State Park demo,
  LDH logo, Virgin Hotels VR, LSU POC render).
- `capturedWith` / `platform` — accurate chips per project.
- Three formerly "Illustrative example" windows now carry real projects:
  Healthcare Training (Immersive Stroke Assessment), Healthcare Facilities
  (CAD→Unity flythrough), Sustainability (ULL EESEC). Heritage remains
  illustrative; Safety & Emergency has copy but no inventory media.

## Script / markup / style support
- `script.js` — `openExample` now loads the example's own media into a dedicated
  `#exampleMediaFrame` (torn down on close so video stops), with the shared
  showcase iframe + sweep navigation kept as the fallback; renders the real
  photo gallery and the related-links chips; "Enter Twin" and the open-in-tab
  square open the ACTIVE example's own tour/video.
- `index.html` — `#exPlatform` chip id, `#exMediaGrid`, `#exMediaLinks`.
- `styles.css` — `img.example-tile-photo`, `.example-links`, `.example-link-chip`.

## Dialog / motto text
- `brand.motto: "The World as Interface"`.
- Question bar: new prompts + answers "What does Digital Twin Studios do?"
  (motto: DTS serves the systems that shape human life; Built. Deployed.
  Awarded.) and "Why Education, Industry, Government, Community?".

## Curation rules applied
- Only Public/Unlisted links used; Private, Inactive, and the item flagged
  "Sensitive Information" (Do It Greener x AICORES) were excluded.
- Unlisted Vimeo links embed with their `h=` hash; `vimeo.com/share/...` and
  `vimeo.com/dtsxr/...` slugs can't be embedded reliably, so they appear as
  external links only.

Headless smoke test: 130/130 checks (all 16 windows open, media frames load,
galleries and link chips render, Enter-Twin routing, teardown on close).
