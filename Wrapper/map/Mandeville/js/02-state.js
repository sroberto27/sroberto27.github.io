...
/* Fontainebleau: tour polygons split by category — recreation
   stops (PLAY, WATERFRONT) render in gold; arrival, history and
   lodging stops in pine green (config.styles.toursPark vs .tours).
   Category comes from locations.json via getCategory(). */
function isParkFeature(feature) {
  ...
  return /play|waterfront|beach|recreation|trail/.test(cat);
}
...
function styleFor(kind, feature) {
  ...
  if (kind === "tour") {
    ...
    if (isParkFeature(feature) && s.toursPark) {
      return { ...s.toursPark };
    }
    return { ...s.tours };
  }
...
function hoverStyleFor(kind, feature) {
  ...
  if (kind === "tour") {
    ...
    if (isParkFeature(feature) && s.toursParkHover) {
      return { ...s.toursParkHover };
    }
    return { ...s.toursHover };
  }
...
function selectedStyleFor(feature) {
  ...
  if (isParkFeature(feature) && s.selectedPark) {
    return { ...s.selectedPark };
  }
  return { ...s.selected };
}
