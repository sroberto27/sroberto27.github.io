/* === App — Part 14: Redesign glue layer (NEW) ======================
...
(function initRedesign() {
  ...
  if (locateBtn) {
    locateBtn.addEventListener("click", () => {
      ...
      map.once("locationerror", () => {
        // Graceful fallback: recenter on the park extent.
  ...
  if (layersBtn) {
    // Placeholder for future park data layers.
    layersBtn.addEventListener("click", () => {
      layersBtn.classList.add("is-flash");
      setTimeout(() => layersBtn.classList.remove("is-flash"), 600);
    });
  }
  ...
  renderDetails = function (feature, kind) {
    ...
    if (shareBtn) {
      shareBtn.onclick = async () => {
        ...
        const payload = { title: `${name} — Fontainebleau State Park Virtual Tour`, url };
        try {
          if (navigator.share) {
