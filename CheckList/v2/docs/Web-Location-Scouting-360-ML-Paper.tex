...
\item We report the first (to the authors' knowledge) measurement showing that pose correction and focal-length correction are \emph{strongly coupled} in this setting: applying either alone recovers under 1.5\,dB of PSNR against ground truth, while applying both together recovers 8.42\,dB -- far more than the sum of the parts.
...
\centering
\small
\begin{tabularx}{\textwidth}{@{}cX>{\raggedright\arraybackslash}p{0.22\textwidth}>{\raggedright\arraybackslash}p{0.30\textwidth}@{}}
\toprule
...
\begin{tikzpicture}[
  ...
  \node[box] (capture) {34 photographs\\ + recorded sensor pose};
...
This design point -- a small, purely convolutional network over a transformer-based matcher such as LightGlue~\cite{lindenberger2023lightglue} -- is a direct consequence of the deployment cost structure. Extraction cost scales linearly in the number of views ($N=34$); matching cost, addressed next, scales with the number of overlapping \emph{pairs} ($\mathcal{O}(N^2)$ in general, though far fewer in practice under the prior gate). A transformer-based matcher performs a learned forward pass \emph{per pair}; on a single-threaded WebAssembly runtime (Section~\ref{sec:constraints}) with roughly eighty to a hundred overlapping pairs in a full 34-shot session, this cost structure favors a design where the expensive learned computation happens once per \emph{view}, and matching itself is a cheap, classical nearest-neighbor search.
...
\bottomrule
\end{tabular}
\caption{Experiment 1: geometric recovery on a controlled synthetic 34-shot capture, synthetic correspondences. Pose error is reported after removing the estimated set's unobservable global gauge rotation by Procrustes alignment on $\SO$, as any two rotation sets differing only by a common rotation are geometrically equivalent panoramas.}
\label{tab:exp1}
\end{table}
...
\midrule
A & Sensor pose + $68\Deg$ (prior behaviour) & 11.90 & 0.168 & 97.9\% \\
B & Refined pose + $68\Deg$ & 13.11 & 0.185 & 98.0\% \\
C & Sensor pose + calibrated focal & 13.26 & 0.300 & 98.3\% \\
D & Refined pose + calibrated focal & \textbf{20.31} & \textbf{0.706} & 98.3\% \\
...
\addplot[fill=blue!35] coordinates {(A,11.90) (B,13.11) (C,13.26) (D,20.31) (E,23.08) (F,26.56)};
\end{axis}
\end{tikzpicture}
...
\textbf{Sphere coverage is a separate axis from alignment quality, and was the subject of two distinct bugs.} No stitching improvement can fill a direction the camera never pointed at, and early real-world captures returned panoramas with conspicuous black holes despite good alignment. Section~\ref{sec:coverage} treats this in full; the short version is that one cause was a blending-weight defect that silently discarded $\approx$21\% of every photograph, and the other was a genuine geometric gap in the original 26-shot capture pattern. The residual shortfall in Table~\ref{tab:exp3} (98.3\%, not 100\%) is simulated per-shot \emph{aiming} error, which is the realistic case: the current pattern measures 100\% coverage when every shot lands exactly on target.

The remaining budget after the full pipeline (D) is $2.77$\,dB of residual pose/focal error relative to the geometric ceiling (E), and a further $3.47$\,dB attributable purely to uncorrected exposure drift (E$\to$F) -- a photometric error no pose or focal correction can touch by construction, which gain compensation (Section~\ref{sec:gain}) is shown here to recover directly.

\subsection{Sphere Coverage: Two Defects That Presented Identically}
\label{sec:coverage}

Once the alignment pipeline was working on real captures, the dominant remaining visual defect was not misalignment but \emph{missing image}: conspicuous black regions, with a distinctive scalloped or petal-like outline, in otherwise well-aligned panoramas. Two independent causes turned out to be responsible, and they are worth separating carefully because they are indistinguishable by eye and only one of them is a capture-pattern issue.

\textbf{Cause 1: the blend weight silently discarded every frame corner.} The compositing feather (Section~\ref{sec:camera}) was radial, computed as $\cos\!\left(\min\!\left(\sqrt{n_x^2+n_y^2},\,1\right)\cdot\pi/2\right)$ over normalized frame coordinates $n_x, n_y \in [-1,1]$. At a frame \emph{corner}, $n_x = n_y = \pm 1$, so the radial distance is $\sqrt{2}\approx1.41$, which clamps to $1$ and yields $\cos(\pi/2)\approx 6\times10^{-17}$ -- numerically zero. Combined with the accumulated-weight threshold used to decide whether an output pixel was covered at all, this discarded everything outside the largest ellipse inscribed in each photograph: approximately $1-\pi/4 \approx 21\%$ of every image captured. The defect is easy to miss on inspection because the expression is a perfectly reasonable \emph{radial} falloff; it is only wrong because the domain is a rectangle, not a disc. The fix is a separable feather, $\cos(|n_x|\pi/2)\cdot\cos(|n_y|\pi/2)$, which still tapers to zero at every edge -- preserving soft seams -- while covering the full rectangular frame.

\textbf{Cause 2: the original capture pattern could not close the sphere.} Independently of any blending defect, the 26-shot pattern (rings at $0\Deg$ and $\pm35\Deg$, with zenith and nadir shots at only $\pm80\Deg$ rather than true poles) left a genuinely unreachable band between the upper ring's top edge and the single zenith shot, across most longitudes. This is a geometric property of the pattern, correctable only by capturing more directions.

Table~\ref{tab:coverage} reports measured sphere coverage for the old and current patterns, computed by equal-area sampling of the sphere and testing each sample against every view using the real projection and the real feather weight -- that is, measuring the pipeline as implemented rather than an idealized rectangular footprint.
...
\toprule
\textbf{Capture pattern} & $58\Deg$ & $62\Deg$ & $68\Deg$ & $73.5\Deg$ & $78\Deg$ & \textbf{shots} \\
\midrule
Original ($0\Deg$, $\pm35\Deg$, zenith/nadir $\pm80\Deg$) & 87.1\% & 90.2\% & 93.6\% & 95.7\% & 96.8\% & 26 \\
\textbf{Current} ($0\Deg$, $\pm33\Deg$, $\pm66\Deg$, true $\pm90\Deg$) & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} & \textbf{100\%} & 34 \\
\bottomrule
\end{tabular}
\caption{Measured sphere coverage as a function of assumed horizontal field of view, with the separable feather in place. The margin across the whole FOV range is the point: the true lens FOV is \emph{unknown at capture time} -- it is calibrated afterwards, from the captured photographs (Section~\ref{sec:calib}) -- so a pattern that only achieves full coverage at the optimistic end of the range will leave real gaps on a narrower lens.}
\label{tab:coverage}
\end{table}

The current pattern adds rings at $\pm66\Deg$ to close the band the old pattern could not reach, moves zenith and nadir to the true poles, and staggers the $\pm33\Deg$ rings by $22.5\Deg$ in yaw relative to the horizon ring so that ring-to-ring overlap does not land corner-to-corner. That stagger has a secondary benefit for the method in this paper: it increases cross-ring overlap, which yields more inter-ring correspondences for the bundle adjustment and therefore a better-connected view graph.

The general lesson generalizes beyond this application: \textbf{coverage and alignment quality are independent axes and should be reported separately}. A metric such as PSNR computed only over covered pixels -- as in Table~\ref{tab:exp3}, deliberately -- says nothing about how much of the sphere was captured, and a system can trivially improve its apparent photometric score by covering less. Conversely, a coverage figure says nothing about whether what was covered is correctly aligned.

\subsection{Two Further Defects Found Only Through Real-Device and Rendering Tests}
...
\textbf{Unfiltered orientation input.} The capture overlay drove both its live world-locked preview and its shot-stability timer directly from raw \texttt{DeviceOrientationEvent} readings, with no smoothing. Consumer orientation sensors carry sample-to-sample noise on the order of a few tenths of a degree, which was enough to make already-captured preview patches visibly shake -- undermining exactly the visual-confirmation affordance the overlay exists to provide -- and to spuriously reset the hold-steady timer. This was resolved with a One Euro filter~\cite{casiez2012oneeuro} applied to yaw, pitch and roll, chosen because its speed-adaptive cutoff gives heavy smoothing while the device is nearly still and progressively less as it moves, which is precisely the aiming-versus-sweeping tradeoff this interaction requires. The filter operates on wrap-safe angular deltas rather than raw angle differences, so it does not produce a spurious transient when yaw crosses $\pm180\Deg$. Measured on synthetic input at 60\,Hz: mean deviation while stationary falls from $0.62\Deg$ to $0.16\Deg$, with a worst-case lag of $5.6\Deg$ during a brisk $90\Deg$/s sweep.

\textbf{Inverted texture orientation in the viewer.} The interactive equirectangular viewer applied WebGL's \texttt{UNPACK\_FLIP\_Y\_WEBGL} on texture upload, which vertically inverted every panorama -- rendering ceiling content underfoot and floor content overhead. The flip is the conventional fix for a full-screen-quad shader sampled by its own vertex UV coordinates, where the $v=0$ convention differs from texture storage order; it is exactly wrong for a shader that, as here, computes a texture coordinate \emph{directly} from spherical longitude and latitude and samples with it as a raw coordinate, because there is no quad-UV convention for the flip to cancel against. The bug is notable mainly for how it evaded review: the resulting panorama is fully populated, correctly aligned, and internally consistent, and reads as plausible until one specifically checks which way is up.
...
\textbf{Method.} Using the paired captures of Section~\ref{sec:val-paired}, additionally reconstruct panoramas under the same six hypotheses (A--F) of Table~\ref{tab:exp3} from each real 34-shot session -- pose correction alone, focal correction alone, both together, and (where a real reference exists) the ground-truth-aligned ceiling -- and re-run the same decomposition. Report the finding as confirmed, attenuated, or contradicted per site category, and specifically test whether the super-additive coupling holds at the same order of magnitude.
...
\item \textbf{Focal length is treated as constant across a session.} The self-calibration of Section~\ref{sec:calib} assumes a single shared focal length across all 34 shots of one session, which is physically correct only if the device does not change optical zoom mid-capture; this is not currently enforced or verified.
...
\item \textbf{Coverage and alignment are conflated in appearance.} Uncovered sphere directions render as black gaps that are easily mistaken for a stitching artifact (Section~\ref{sec:coverage}); any human perceptual study (Section~\ref{sec:val-perceptual}) must be designed so raters are not penalizing the method for a capture-pattern limitation it does not address. Coverage should be reported alongside PSNR/SSIM rather than folded into them.
\end{itemize}
...
G. Casiez, N. Roussel, and D. Vogel, ``1 Euro Filter: A Simple Speed-Based Low-Pass Filter for Noisy Input in Interactive Systems,'' in \textit{Proc.\ SIGCHI Conf.\ on Human Factors in Computing Systems (CHI '12)}, Austin, TX, USA, 2012, pp.~2527--2530. DOI: 10.1145/2207676.2208639.
